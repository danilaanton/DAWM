...
export class HomeComponent {
  ...
  constructor(
    ...
    private imageCrud: ImageCrudService,
  ...
  loadBatch() {
    ...
    for (
      let i = this.loadedImages.length;
      i < Math.min(this.shownMetadata.length, targetIndex);
      i++
    ) {
      ...
      this.userService
        .getUser(
          this.shownMetadata[i].author.substring(
            1,
            this.shownMetadata[i].author.length - 1
          )
        )
        .subscribe((res) => {
  ...
  handleFilter(event: string) {
    this.loadedImages = [];
    console.log(event);
    this.shownMetadata = this.imageMetadata.filter(
      (image) => image.author === '"' + event + '"'
    );
    this.loadBatch();
  }
}
